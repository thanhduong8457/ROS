...
def espaciotrabajo(deltaangulo):
 ...
 mi_path = "/mnt/e/catkin_ws/src/simu_visual/script/workspace.txt"
 ...
 res_ang_min = delta_define.res_ang_min()
 res_ang_max = delta_define.res_ang_max()
 ...
 for theta11 in range(res_ang_min, res_ang_max, deltaangulo):
  for theta12 in range(res_ang_min, res_ang_max, deltaangulo):
   for theta13 in range(res_ang_min, res_ang_max, deltaangulo):
	...
	theta2i_min = delta_define.theta2i_min()
	theta2i_max = delta_define.theta2i_max()
	theta3i_min = delta_define.theta3i_min()
	theta3i_max = delta_define.theta3i_max()
	...
	if (theta2i_min < theta21 < theta2i_max and
	 ...
	 if (theta3i_min < theta31 < theta3i_max and
	  ...
	  # Error singularidad
	  err_jxx = delta_define.err_jxx()
	  err_jinv = delta_define.err_jinv()
	  ...
	  if (det_jxx > err_jxx or det_jxx < (-1 * err_jxx)) and (
	   ...
	   # ****** {{{ Restriccion Cubo}}} ******
	   # *************************************
	   if (((px < delta_define.px_max_ws()) and
			(px > delta_define.px_min_ws()))
		 and ((py < delta_define.py_max_ws()) and
			  (py > delta_define.py_min_ws()))
		 and ((pz < delta_define.pz_max_ws()) and
			  (pz > delta_define.pz_min_ws()))):
